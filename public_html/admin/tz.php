...
$Page->form[]=new textType('name','Текст',true);
$Page->form[]=new fileType('path','Файл pdf,doc', $Page->table,false,false,array("pdf","doc"));
...
if(isset($_GET['add'])){
	...
	foreach ($Page->form as $formi) {
			...
			else if($formi->type=="file"){

				if (is_uploaded_file($_FILES[$formi->name]['tmp_name'])) {	
					$filename = $_FILES[$formi->name]['name'];
					$path_info = pathinfo($filename);
					$extension = $path_info['extension'];
					if($formi->check($extension)){
						$file = rus_to_eng($path_info['filename'])."-".time().round(microtime()*1000).".".$extension;	
						move_uploaded_file($_FILES[$formi->name]['tmp_name'], $file);
						$values[$formi->name]=$file;
					}else $_SESSION['notice'].="<br><span style='color:#f00'>Файл неверного формата</span>";
				}
			}
			else{
				$values[$formi->name]=$formi->get($_POST[$formi->name]);
			}
	}
